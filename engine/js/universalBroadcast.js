...
async function getNewsFromCDN() {
    ...
    if (newsObj == null || newsObj == undefined) {
        ...
        try {
            // Traverse the array "vars_universalBroadcast.news" and query the CDN for each news (each URL)
...
async function getNewsSchemaFromCDN() {
    ...
    if (newsSchema == null || newsSchema == undefined) {
        ...
        try {
            // Traverse the array "vars_universalBroadcast.newsSchema" and query the CDN for each news schema (each URL)
...
async function broadcastNews() {
    ...
    try {
        ...
        // "news" is an array of objects, each object representing a news.
        // Traverse all the news following the format above
        for (var z = 0; i < newsObj.length; z++) {
            // Traverse all the news following the format above
